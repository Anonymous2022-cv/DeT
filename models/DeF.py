...
import numpy as np
from models.arch_util import (ResidualBlockNoBN, make_layer, RCAB, ResidualGroup, default_conv, RCABWithInputConv)
...
from models.spynet import SPyNet, SPyNetBasicModule, ResidualBlocksWithInputConv
...
class Transformer(nn.Module):
    def __init__(self, num_feat, feat_size, depth, patch_size, heads, n_FusionB):
        super().__init__()
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                Residual(PreNorm(num_feat, feat_size, Fusion(n_FusionB, num_feat)))
            ]))
